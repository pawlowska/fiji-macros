 ...
 * - overlap value
 * runs "Show Info..." and saves it as image_info.txt
 */
print("\\Clear");
...
// Show Info save
setBatchMode(true);
list_f = getFileList(pth);
Array.sort(list_f);
open(pth+list_f[0]);
run("Show Info...");
saveAs("text", pth_output+File.separator+"image_info.txt");
//run("Close");
print("Image info from "+list_f[0]+" was saved.");
